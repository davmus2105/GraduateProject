...
namespace LunarCatsStudio.SuperCombiner {
	public class MeshCombiner {
        ...
        /// <summary>
        /// The reference to the combinedResult
        /// </summary>
        ...
        public List<GameObject> CombineToMeshes(List<MeshRenderer> meshRenderers, List<SkinnedMeshRenderer> skinnedMeshRenderers, Transform parent, int combinedIndex)
        {
            ...
            for (int i = 0; i < meshRenderers.Count; i++)
            {
                ...
                combineInstances.AddRange(CreateCombinedInstances(newMesh, meshRenderers[i].sharedMaterials, meshRenderers[i].gameObject.GetInstanceID(), meshRenderers[i].gameObject.name, matrix, combinedIndex));
            }
        ...
        private CombineInstanceID CreateCombinedInstances(Mesh mesh, Material[] sharedMaterials, int instanceID, string name, Matrix4x4 matrix, int combinedIndex)
        {
			...
			for (int k = 0; k < mesh.subMeshCount; k++) {
                // Find corresponding material for each submesh
                if (k < sharedMaterials.Length)
                {
                    Material mat = sharedMaterials[k];
                    textureIndexes[k] = combinedResult.FindCorrespondingMaterialIndex(mat, combinedIndex);
                } else
                {
            ...
            {
                GenerateUV(mesh, textureIndexes, combinedResult.combinedMaterials[combinedIndex].scaleFactors.ToArray(), name, combinedIndex);
            }
		...
		public GameObject CreateCombinedMeshGameObject(CombineInstanceID instances, Transform parent, int number, int combinedIndex)
        {
            ...
            {                
				combined = new GameObject(sessionName + "_" + combinedResult.combinedMaterials[combinedIndex].displayedIndex + "_" + number.ToString());
		...
		{		
			...
			if (new_uv.Length > 0) {
				for (int i = 0; i < subMeshCount; i++) {
                    ...
                    if (textureIndex[i] < combinedResult.combinedMaterials[combinedIndex].uvs.Length) {
                        uvsInAtlasTexture[i] = combinedResult.combinedMaterials[combinedIndex].uvs[textureIndex[i]];
                        ...
                        for (int j = 0; j < subMeshTriangles.Length; j++) {
                            ...
                            new_uv [uvIndex].x -= combinedResult.combinedMaterials[combinedIndex].meshUVBounds [textureIndex [i]].xMin;
							new_uv [uvIndex].y -= combinedResult.combinedMaterials[combinedIndex].meshUVBounds [textureIndex [i]].yMin;

                            // Scale (if necessary) new mesh's uvs so that it fits in a (1, 1) square
                            if (combinedResult.combinedMaterials[combinedIndex].meshUVBounds [textureIndex [i]].width != 0 && combinedResult.combinedMaterials[combinedIndex].meshUVBounds [textureIndex [i]].width != 1) {
								new_uv [uvIndex].Scale (new Vector2 (1 / combinedResult.combinedMaterials[combinedIndex].meshUVBounds [textureIndex [i]].width, 1));
							}
							if (combinedResult.combinedMaterials[combinedIndex].meshUVBounds [textureIndex [i]].height != 0 && combinedResult.combinedMaterials[combinedIndex].meshUVBounds [textureIndex [i]].height != 1) {
								new_uv [uvIndex].Scale (new Vector2 (1, 1 / combinedResult.combinedMaterials[combinedIndex].meshUVBounds [textureIndex [i]].height));
							}
			...
			if (uv2 != null && uv2.Length > 0 && combinedResult.combinedMaterials[combinedIndex].uvs2 != null && combinedResult.combinedMaterials[combinedIndex].uvs2.Length > 0) {
				for (int l = 0; l < uv2.Length; l++) {
					new_uv2 [uv2.Length + l] = new Vector2((uv2[l].x * combinedResult.combinedMaterials[combinedIndex].uvs2[textureIndex[0]].width) + combinedResult.combinedMaterials[combinedIndex].uvs2[textureIndex[0]].x, (uv2[l].y * combinedResult.combinedMaterials[combinedIndex].uvs2[textureIndex[0]].height) + combinedResult.combinedMaterials[combinedIndex].uvs2[textureIndex[0]].y);
				}
