...
namespace LunarCatsStudio.SuperCombiner 
{
	...
	public class Saver {
		...
		/// Saves the textures.
		/// </summary>
		/// <param name="combinedIndex">Combined index.</param>
		...
		public static void SaveMaterial(int combinedIndex, string folderDestination, string sessionName, TexturePacker texturePacker)
		{
			...
			Material materialToSave = AssetDatabase.LoadAssetAtPath<Material> (folderDestination + "/Materials/" + texturePacker.copyedMaterials.name + ".mat");

			if (materialToSave == null) {
				AssetDatabase.CreateAsset (texturePacker.copyedToSaveMaterials, folderDestination + "/Materials/" + texturePacker.copyedMaterials .name + ".mat");
				AssetDatabase.SaveAssets ();
				AssetDatabase.Refresh ();
			} else {
				EditorUtility.CopySerialized (texturePacker.copyedToSaveMaterials, materialToSave);
			}

			Material material = (Material)(AssetDatabase.LoadAssetAtPath (folderDestination + "/Materials/" + texturePacker.copyedMaterials.name + ".mat", typeof(Material)));
		...
		public static void SaveCombinedResults(CombinedResult combinedResult, string folderDestination, string sessionName) 
		{
			...
			for(int i=0; i<combinedResult.combinedMaterials.Count; i++) {
				if (combinedResult.combinedMaterials [i].material != null) {
					toSaveCombinedResult.combinedMaterials[i].material = (Material)(AssetDatabase.LoadAssetAtPath (folderDestination + "/Materials/" + combinedResult.combinedMaterials [i].material.name + ".mat", typeof(Material)));
				}
			}
